...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=horizon_cisco_ui/cisco/test/integration_tests"
  fi
...
function run_tests_all {
  ...
  ${command_wrapper} ${coverage_run} $root/manage.py test horizon_cisco_ui.cisco --settings=openstack_dashboard.test.settings $testopts
