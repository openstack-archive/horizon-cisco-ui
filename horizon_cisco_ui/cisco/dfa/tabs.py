...
class FabricSummaryTab(tabs.TableTab):
    ...
    def get_fabricsummary_data(self):
        summary = []
        try:
            if not bool(client.__dict__):
                return summary
            summary = client.get_fabric_summary()
...
class CFGProfileTab(tabs.TableTab):
    ...
    def get_cfgprofile_data(self):
        try:
            if not bool(client.__dict__):
                return []
            cfgplist = client.get_config_profiles_detail()
...
class NFEInfoTab(tabs.TableTab):
    ...
    def get_projecttable_data(self):
        try:
            ...
            project_list = client.get_project_details(tenant)
        except Exception as exc:
            project_list = []
            exceptions.handle(self.request, exc.message)
        return project_list
    ...
    def get_networktable_data(self):
        try:
            if not bool(client.__dict__):
                return []
            tenant = dict(tenant_id=self.request.user.project_id)
            netlist = client.get_network_by_tenant_id(tenant)
        except Exception as exc:
            netlist = []
            exceptions.handle(self.request, exc.message)
        return netlist
    ...
    def get_instancetable_data(self):
        try:
            if not bool(client.__dict__):
                return []
            tenant = dict(tenant_id=self.request.user.project_id)
            instance_list = client.get_instance_by_tenant_id(
                tenant)
            agent_list = client.get_agents_details()
            port = {}
            for agent in agent_list:
                if agent.get('agent_status') == 'Not Active':
                    continue
...
class NFEAgentTab(tabs.TableTab):
    ...
    def get_agentstable_data(self):
        try:
            ...
            for agent in agent_list:
                agent["heartbeat"] = agent.get('heartbeat').replace('T',
                                                                    ' ')[:-7]
