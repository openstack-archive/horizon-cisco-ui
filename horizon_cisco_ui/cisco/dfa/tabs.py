...
class FabricSummaryTab(tabs.TableTab):
    ...
    def get_fabricsummary_data(self):
        try:
            ...
            summary = dfaclient.get_fabric_summary()
...
class NFEInfoTab(tabs.TableTab):
    ...
    def get_projecttable_data(self):
        try:
            ...
            project_list = dfa_client.dfa_client().get_project_details(tenant)
        except Exception as exc:
            project_list = []
            exceptions.handle(self.request, exc.message)
        return project_list
    ...
    def get_networktable_data(self):
        try:
            tenant = dict(tenant_id=self.request.user.project_id)
            netlist = dfa_client.dfa_client().get_network_by_tenant_id(tenant)
        except Exception as exc:
            netlist = []
            exceptions.handle(self.request, exc.message)
        return netlist
    ...
    def get_instancetable_data(self):
        try:
            tenant = dict(tenant_id=self.request.user.project_id)
            instance_list = dfa_client.dfa_client().get_instance_by_tenant_id(
                tenant)
            agent_list = dfa_client.dfa_client().get_agents_details()
...
class NFEAgentTab(tabs.TableTab):
    ...
    def get_agentstable_data(self):
        try:
            dfaclient = dfa_client.dfa_client()
            agent_list = dfaclient.get_agents_details()
