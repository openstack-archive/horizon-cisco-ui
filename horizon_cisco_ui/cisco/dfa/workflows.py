...
class DFAConfigProfileAction(workflows.Action):
    ...
    def _get_cfg_profiles(self, request):
        ...
        try:
            ...
            cfgplist = dc.get_config_profiles_detail()
    ...
    '''def clean(self):
        cleaned_data = super(DFAConfigProfileAction, self).clean()
        cfgp = cleaned_data.get('cfg_profile')
        if not cfgp:
...
class DFACreateNetwork(upstream_networks_workflows.CreateNetwork):
    ...
    def handle(self, request, data):
        ...
        if data['cfg_profile']:
            dc = dfa_client.dfa_client()
            try:
                precreate_flag = dc.do_precreate_network(precreate_network)
            except Exception as exc:
